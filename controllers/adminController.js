...
const rechargeFailedPage = async (req, res) => {
  return res.render("manage/rechargefailed.ejs");
};


const withdrawFailPage = async (req, res) => {
  return res.render("manage/withdrawFailed.ejs");
};
...
const p2pPage = async (req, res) => {
  return res.render("manage/p2p.ejs");
};

const p2pMainPage = async (req, res) => {
  return res.render("manage/p2pmain.ejs");
};
  ...
  try {
    ...
    const routePermissions = {
      ...
      "/admin/manager/withdrawRecord": "Withdrawal (Approved)",
            "/admin/manager/withdrawfailed": "Withdrawal (failed)",
      ...
      "/admin/manager/p2p":"P2p", 
      "/admin/manager/p2pmain":"P2P Account"
...
const editbank = async (req, res) => {
  ...
  console.log("upi",upi, req.body)
  try {
    let a = await connection.query(
      "UPDATE user_bank SET name_bank=?,name_user=?,email=?,stk=?,upi=?, WHERE id = ? ",
      [bank_name, name, ifsc, stk, upi, id]
    );
    // res.send(id);
    res.redirect("back");
  } catch (e) {
...
const editbankstatus = async (req, res) => {
  let phone = req.body.phone;
  let verify = req.body.verify
  try {
    let a = await connection.query(
      "UPDATE user_bank SET verify=? WHERE phone = ?",
      [verify,phone]
    );
    res.send("success");
    // res.redirect("back");
...
const recharge = async (req, res) => {
  try {
    ...
    // PENDING RECHARGE
    if (type === "pendigrecharge") {
      const [data] = await connection.query(
        "SELECT * FROM recharge WHERE status = 0 AND isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      recharge = data;
    }

    // SUCCESS RECHARGE
    if (type === "successrecharge") {
      const [data] = await connection.query(
        "SELECT * FROM recharge WHERE status = 1 AND isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      successFullRecharge = data;
    }

    // FAILED RECHARGE
    if (type === "failrecharge") {
      const [data] = await connection.query(
        "SELECT * FROM recharge WHERE status = 2 AND isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      faileRecharge = data;
    }
    ...
    if (type === "allrecharge") {
      const [data] = await connection.query(
        "SELECT * FROM recharge WHERE isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      allRecharge = data;
    }

    // SUCCESS + FAILED (status != 0)
    if (type === "successfaild") {
      const [data] = await connection.query(
        "SELECT * FROM recharge WHERE status != 0 AND isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      successfaild = data;
    ...
    if (type === "withdrawpending") {
      const [data] = await connection.query(
        "SELECT * FROM withdraw WHERE status = 0 AND isdemo=0 ORDER BY id DESC LIMIT 100"
      );
      withdraw = data;
    }
    ...
    if (type === "withdraw2") {
      const [data] = await connection.query(
        "SELECT * FROM withdraw WHERE status = 1 AND isdemo=0" +
          wherequery +
          " ORDER BY id DESC LIMIT 100"
      );
      withdraw2 = data;
    ...
    // withdrawfailed
    let withdraw3 = [];
    if (type === "withdrawfailed") {
      const [data] = await connection.query(
        "SELECT * FROM withdraw WHERE status = 2 AND isdemo=0" +
          wherequery +
          " ORDER BY id DESC LIMIT 100"
      );
      withdraw3 = data;
    ...
    return res.status(200).json({
      ...
      datas3: withdraw,
      datas4: withdraw2,
      withdrawfailed: withdraw3,
      successfaild: successfaild,
    });
  } catch (error) {
...
const rechargeDuyet = async (req, res) => {
  ...
  try {
    if (type === "confirm") {
      ...
      const phone = rechargeInfo.phone;
      ...
      const [firstRecharge] = await connection.query(
        "SELECT id FROM recharge WHERE phone = ? AND status = 1",
        [phone]
      );

      const isFirstDeposit = firstRecharge.length === 0;

      // =============================
      // ✅ FIRST DEPOSIT BONUS
      // =============================
      if (isFirstDeposit) {
        const [bonusTable] = await connection.query(
          "SELECT recAmount, bonus, is_persent FROM rechargeBonus WHERE is_fisrtrecharge = 1 ORDER BY recAmount ASC"
        );

        for (const row of bonusTable) {
          const recAmount = parseFloat(row.recAmount);

          if (rechargeInfo.money >= recAmount) {
            if (parseInt(row.is_persent) === 1) {
              firstDepositBonus = (rechargeInfo.money * parseFloat(row.bonus)) / 100;
            } else {
              firstDepositBonus = parseFloat(row.bonus);
            }
          }
        }
        ...
        if (firstDepositBonus > 0) {
          await connection.query(
            ...
            [phone, "First deposit bonus", firstDepositBonus, checkTime]
          );
        }
      }

      // =============================
      // ✅ USDT 2% BONUS (USER ONLY)
      // =============================
      if (rechargeInfo.type === "USDT") {
        usdtBonus = rechargeInfo.money * 0.02;

        await connection.query(
          "INSERT INTO transaction_history (phone, detail, balance, time) VALUES (?, ?, ?, ?)",
          [phone, "USDT 2% bonus", usdtBonus, checkTime]
        );
      }

      totalUserBonus = firstDepositBonus + usdtBonus;

      // =============================
      // ✅ SPONSOR BONUS ONLY FOR FIRST DEPOSIT
      // =============================
      const sponsorCode = userInfo[0]?.invite;

      if (sponsorCode) {
        const [sponsor] = await connection.query(
          "SELECT phone FROM users WHERE code = ? LIMIT 1",
          [sponsorCode]
        );

        if (sponsor.length > 0) {
          const sponsorPhone = sponsor[0].phone;

          // ✅ Only first deposit bonus goes to sponsor
          if (firstDepositBonus > 0) {
            await connection.query(
              "UPDATE users SET money = money + ?, total_money = total_money + ?, recharge = recharge + ? WHERE phone = ?",
              [firstDepositBonus, firstDepositBonus, firstDepositBonus, sponsorPhone]
            );

            await connection.query(
              "INSERT INTO transaction_history (phone, detail, balance, time) VALUES (?, ?, ?, ?)",
              [sponsorPhone, "Referral bonus (First deposit)", firstDepositBonus, checkTime]
            );
          }

          // ❌ Removed: sponsor USDT 2% bonus
        }
      }
      ...
      if (totalUserBonus > 0) {
        await connection.query(
          "UPDATE users SET money = money + ?, total_money = total_money + ?, recharge = recharge + ? WHERE phone = ?",
          [totalUserBonus, totalUserBonus, totalUserBonus, phone]
        );
      }
      ...
      if (spinData.length > 0 && rechargeInfo.money >= spinData[0].task_amount) {
        const totalSpins =
          Math.floor(rechargeInfo.money / spinData[0].task_amount) *
          spinData[0].num_of_spin;

        await connection.query(
          "UPDATE users SET total_spin = total_spin + ?, spin_recharge = spin_recharge + ? WHERE phone = ?",
          [totalSpins, rechargeInfo.money, phone]
        );
      }
      ...
      await connection.query(
        ...
        [rechargeInfo.money, rechargeInfo.money, rechargeInfo.money, rechargeInfo.money, phone]
      );

      // =============================
      // ✅ ADD MAIN DEPOSIT RECORD
      // =============================
      await connection.query(
        "INSERT INTO transaction_history (phone, detail, balance, time) VALUES (?, ?, ?, ?)",
        [phone, "Deposit", rechargeInfo.money, checkTime]
      );
    ...
    await connection.query(`UPDATE withdraw SET status = 1 WHERE id = ?`, [id]);
        ...
        return res.status(200).json({
         ...
         if(info[0].type==="UPI"){
          ...
          if(info[0].type==="P2P Wallet"){
         
    await connection.query(`UPDATE withdraw SET status = 1 WHERE id = ?`, [id]);
        await connection.query(`INSERT INTO transaction_history SET phone=?, detail=?,balance=?, time = ?`,[info[0].phone,"P2P Withdraw",info[0].money,checkTime])
        await connection.query('UPDATE users SET p2pwallet = p2pwallet + ? WHERE phone = ? ', [info[0].money, info[0].phone]);
...
const payoutData = {
  amount: info[0].money,
  auth: 'YUTMH4E1YAJQWIA5J92T', // Replace with your actual merchant key
...
const response=await axios.post('https://api.zilpay.live/api/payout', payoutData, {   headers: {
...
const unblockLigaltrade = async (req, res) => {
  try {
    ...
    await  connection.execute(
          'UPDATE `users` SET `legal_bet_score` = 0 WHERE `phone` = ?',
          [phone]
        )
    
    const data = await connection.query(
      'DELETE FROM ligal_trade WHERE phone=?',
      [phone]
    );
