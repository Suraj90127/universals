...
function CalendarUpdate({ onDateSelect, selectedDates = [] }) {
        ...
        <div className="grid grid-cols-7 gap-0 relative">
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <div
                key={`day-${day}`}
                className={`h-16 flex flex-col rounded items-center gray-50 justify-center text-md cursor-pointer
                  ${(() => {
                    if (internalSelectedDates.length === 2) {
                      const [start, end] = internalSelectedDates;
                      if (
                        ...
                        return "bg-gray-600 gray-50 rounded my-0.5 ";
                      }
                      if (date > start && date < end) {
                        return "bg-gray-500 text-white my-0.5";
                      }
                    } else if (
                      internalSelectedDates.length === 1 &&
                      date.getTime() === internalSelectedDates[0].getTime()
                    ) {
                      return "bg-gray-600 gray-50 my-0.5 ";
                    }
                    return "";
                  })()}
              ...
              >
                ...
                {internalSelectedDates.length > 0 &&
                  date.getTime() === internalSelectedDates[0].getTime() && (
                    <span className="text-xs gray-50 ">Start</span>
                  )}

                {internalSelectedDates.length === 2 &&
                  date.getTime() === internalSelectedDates[1].getTime() && (
                    <span className="text-xs gray-50 ">End</span>
                  )}
              </div>
            );
          })}
  ...
  return (
    <div>
      <div
        className="bg-body cursor-pointer flex justify-between items-center p-2 rounded-md  "
      ...
      {isOpen && (
        <>
          ...
          <div className="fixed inset-x-0 bottom-0 bg-body max-w-[25.7rem] mx-auto z-50 rounded-t-3xl shadow-lg">
