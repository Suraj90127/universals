...
export const everyVipClame = createAsyncThunk(
  "auth/everyVipClame",
  async (_, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get(`/webapi/vipLevelEverycron`, {
        withCredentials: true,
      });
      console.log("Fetched Data:", data); // Debug API response
      return fulfillWithValue(data);
    } catch (error) {
      console.error("Error in everyVipClame:", error); // Debug API error
      return rejectWithValue(
        error.response ? error.response.data : "An unknown error occurred"
      );
    }
...
export const climeSpinWheelData = createAsyncThunk(
  ...
  async (_, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get(`/webapi/clime-spin-data`, {
        withCredentials: true,
      });
...
export const reddempinWheelSpin = createAsyncThunk(
  ...
  async (_, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get(`/webapi/getweeldetails`, {
        withCredentials: true,
      });
      console.log("Fetched Data:", data); // Debug API response
      return fulfillWithValue(data);
...
export const activityReducer = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(everyVipClame.pending, (state) => {
        state.loader = true;
      })
      .addCase(everyVipClame.rejected, (state, { payload }) => {
        state.errorMessage = payload?.errorMessage || "An error occurred";
        state.loader = false;
      })
      .addCase(everyVipClame.fulfilled, (state, { payload }) => {
        // console.log("Fulfilled payload:", payload);
        state.successMessage = payload.message || "";
        state.vipData = payload?.data || [];
        state.loader = false;
