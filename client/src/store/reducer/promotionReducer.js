  ...
  async (category, { rejectWithValue, fulfillWithValue }) => {
    try {
      const url = category && category !== "All" 
        ? `/webapi/transactionhistory?detail=${category.toLowerCase()}`
        : "/webapi/transactionhistory";
...
export const transactionCategory = createAsyncThunk(
  "auth/transaction-category",
  async (_, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.get("/webapi/transactioncatgory", {
        withCredentials: true,
      });
      return fulfillWithValue(data);
    } catch (error) {
...
export const promotionReducer = createSlice({
  ...
  extraReducers: (builder) => {
    builder         
      ...
      .addCase(transactionCategory.pending, (state) => {
        state.loader = true;
      })
      .addCase(transactionCategory.rejected, (state, { payload }) => {
        state.errorMessage = payload?.errorMessage || "An error occurred";
        state.loader = false;
      })
      .addCase(transactionCategory.fulfilled, (state, { payload }) => {
        // console.log("payload",payload);
        
       
        state.successMessage = payload.message;
        state.transactionCategoryData = payload.data||[];
        state.loader = false;
        // state.transactionCategoryData = transactionCategoryData;
      })
