...
const TeamReport = () => {
  ...
  let filteredData = userData?.filter((item) => {
    ...
    const matchesId =
      searchInput !== null ? item?.userId?.toString().includes(searchInput) : true; // Show all if no searchInput is provided
      ...
      <div className="container-section">
        <div className="flex bg-light rounded-md mt-2 justify-between p-2">
          <input
            ...
            className="placeholder:text-white text-sm font-medium bg-transparent focus:outline-none p-1"
            value={searchInput}
            onChange={handleSearchChange}
          />
          <button className="blue-linear rounded-3xl px-5 mr-2 ">
        ...
        <div className="grid grid-cols-12 gap-2 mt-2">
          ...
          >
            <span className="text-base gray-text" onClick={handleOpenAll}>
              {searchLevels}
            </span>
            <span>
        ...
        <div className="mt-2 sheet_nav_bg p-2 rounded-lg">
  {/* Deposit Section */}
  <div className="flex justify-between mt-2">
    <div className="text-center w-[50%]">
      ...
      <p className="fs-sm gray-text">Deposit number</p>
    </div>
    <div className="text-center w-[50%] border-l border-slate-500">
      ...
      <p className="fs-sm text-white">Deposit amount</p>
    </div>
  </div>
    ...
    <div className="text-center w-[50%] border-l border-slate-500">
      <h5 className="heading-h5 text-lg text-white">
        {searchLevels === "All"
          ...
          : Number(filteredDataLevel?.totalBetAmount).toFixed(2) || 0}
      </h5>
  ...
  <div className="flex justify-between mt-3">
    <div className="text-center w-[50%] px-4">
      ...
      <p className="fs-sm text-white">Number of first deposits</p>
    </div>
    <div className="text-center w-[50%] border-l border-slate-500">
      <h5 className="heading-h5 text-lg text-white">
        {searchLevels === "All"
  ...
  <div className="flex justify-between mt-3">
    <div className="text-center w-[50%]">
      <h5 className="heading-h5 text-lg text-white">
        {searchLevels === "All"
          ? subordinateData?.p2p_count || 0
          : filteredDataLevel?.p2p_count || 0}
      </h5>
      <p className="fs-sm text-white">P2P Count</p>
    </div>
    <div className="text-center w-[50%] border-l border-slate-500">
      <h5 className="heading-h5 text-lg text-white">
        {searchLevels === "All"
          ? subordinateData?.p2p_amount || 0
          : filteredDataLevel?.p2p_amount || 0}
      </h5>
      <p className="fs-sm text-white">P2P Amount</p>
    </div>
  </div>
</div>
    ...
    filteredData.map((item, i) => (
      <div className="nav-bg rounded-md mt-3 pb-7 px-3 pt-1" key={i}>
        ...
        <div className="mt-3 flex justify-between items-center rounded-sm text-sm pb-2 border-b border-[--grey-200]">
          <span className="text-sm font-medium text-white p-1 flex items-center">
            UID: {item?.userId}{" "}
            <PiCopySimpleBold
        ...
        {/* 🆕 P2P Amount */}
        <div className="mt-3 flex justify-between items-center text-white text-sm">
          <span className="text-sm font-medium">P2P Amount</span>
