...
const bgspin = "https://i.ibb.co/nMK6w3j5/bg-spin-image-393b4fbdfb7b1c18f0e2.png";
const WheelSpin = () => {
  ...
  // Preload the background image to reduce loading time
  useEffect(() => {
    const img = new Image();
    img.src = bgspin;
    const img2 = new Image();
    img2.src = bgfrontspin;
  }, []);

  useEffect(() => {
    console.log("🌀 useEffect triggered - getSpinWheelDatas:", getSpinWheelDatas);
  
    if (getSpinWheelDatas && getSpinWheelDatas?.rows?.length > 0) {
      const spinData = getSpinWheelDatas?.rows[0];
      console.log("🎯 Spin Data (first row):", spinData);
  
      // Log redeemSpinData for debugging
      console.log("💎 Redeem Spin Data:", redeemSpinData);
  
      // Safely set remaining spins
      const remaining = parseInt(redeemSpinData?.redeamspin) || 0;
      console.log("🎮 Remaining Spins:", remaining);
      ...
      console.log("🏆 Prize List:", newPrizes);
      setPrizes(newPrizes);
    } else {
      console.warn("⚠️ No spin data available or rows are empty.");
      setRemainingSpins(0);
    }
  }, [getSpinWheelDatas]);
    ...
    <div className="min-h-screen">
      ...
      <div className=" p-4">
        <div className="bg-light p-4 rounded-xl shadow">
          {/* Total Recharge */}
          <div className="flex items-center justify-between sheet_nav_bg p-3 rounded-lg">
          ...
          {/* Number of Spins */}
          <div className="flex items-center justify-between sheet_nav_bg p-3 rounded-lg mt-4">
            <h3 className="text-sm font-semibold text-whites">
              Number of Spins
            </h3>
            <div>
        ...
        {/* Next Page Links */}
        <div className="text-[#c4933f] flex items-center justify-between gap-5 mt-10">
        ...
        <div>
          <div className="mt-7 flex items-center justify-start gap-2">
            <IoDocumentTextSharp className="text-[#c4933f] text-[30px]" />
          ...
          {spinhistory.length > 0 ? (
            spinhistoryData?.map((item, i) => (
              <div
                key={i}
                className="bg-body mt-1 flex items-center justify-between px-6 py-2 gray-50 text-sm"
              >
                <div>{item.time}</div>
                <div>{item.type}</div>
                <div>{item.amount}</div>
