          ...
          filteredData?.map((item, i) => (
            <div className="bg-body rounded-md pb-7 mb-4 p-2" key={i}>
              <div className="flex justify-between items-center px-2 text-sm py-1 rounded-sm sky-border border-b">
                <span className="fs-sm font-medium bg-red-500 p-2 px-4 rounded-md text-white">
                  Deposit
                </span>
                <span
              ...
              {item.status === 0 && (
                <Link
                  to={`https://h5.workorder.support.aaladin.pro/deposite?token=${Cookies.get(
                    "auth"
                  )}`}
                  className="blue-linear w-full rounded-full color-orange py-1 mt-3 text-lg font-semibold flex justify-center items-center"
                >
                  Submit Receipt
                </Link>
              )}
